StatsBase.vcov(rm::RegressionModel) = scale(rm,true) * unscaledvcov(rm)
...
StatsBase.coef(nl::NonlinearLS) = copy(nl.pars)

## returns the coefficient table
function StatsBase.coeftable(nl::NonlinearLS)
...
StatsBase.deviance(nl::NonlinearLS) = nl.rss
    ...
    if !nl.fit
        ...
        for i in 1:nl.mxiter
            ...
            if verbose
                ...
                print(", cvg = "); showcompact(cvg); print(" at [");
                showcompact(pars[1])
                for i in 2:length(pars); print(","); showcompact(pars[i]);end
                println("]")
            end
            ...
            if verbose
                print("   Incr: [")
                showcompact(incr[1])
                for i in 2:length(incr); print(","); showcompact(incr[i]);end
                println("]")
            end
            f = 1.
            while true
...
StatsBase.nobs(nl::NonlinearLS) = size(nl,2)
pnames(nl::NonlinearLS) = pnames(model(nl))

function Base.show{T<:FP}(io::IO, nl::NonlinearLS{T})
    gnfit(nl)
    p,n = size(nl)
...
function StatsBase.deviance{T<:FP}(pl::PLinearLS{T},nlp::Vector{T})
...
StatsBase.deviance(pl::PLinearLS) = pl.rss
...
function gpinc{T<:FP}(pl::PLinearLS{T})
    ...
    LAPACK.gemqrt!('L','T',mqr.factors,mqr.T.data,B)
    ...
    if !pl.fit
        ...
        for i in 1:pl.mxiter
            ...
            if verbose
                ...
                print(", cvg = "); showcompact(cvg); print(" at [")
                showcompact(pl.pars[1])
                for i in 2:length(pl.pars); print(","); showcompact(pl.pars[i]); end
                println("]")
                print("   Incr: [")
                showcompact(pl.incr[1])
                for i in 2:length(pl.incr); print(","); showcompact(pl.incr[i]); end
                println("]")
            end
            f = 1.
            while true
...
## returns a copy of the current parameter values
StatsBase.coef(pl::PLinearLS) = copy(pl.pars)

## returns the coefficient table
function StatsBase.coeftable(pl::PLinearLS)
...
StatsBase.fit(m::PLregModF,verbose=false) = gpfit(PLinearLS(m),verbose)

StatsBase.model_response(pl::PLinearLS) = model_response(pl.m)

npars(pl::PLinearLS) = npars(pl.m)

function Base.scale(pl::PLinearLS,sqr=false)
    scsq = deviance(pl)/df_residual(pl)
    sqr ? scsq : sqrt(scsq)
end

StatsBase.residuals(pl::PLinearLS) = residuals(pl.m)

function Base.show{T<:FP}(io::IO, pl::PLinearLS{T})
    gpfit(pl)
...
StatsBase.vcov(pl::PLinearLS) = scale(pl,true) * unscaledvcov(pl.m)
